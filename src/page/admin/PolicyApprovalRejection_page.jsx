...
function PolicyApprovalRejection_page() {
  ...
  const handleApproved = async () => {
    try {
      const response = await axiosInstance.put(`policy_approved/${PolicyId}`)

      if (response.status === 200) {
        toast.success("Policy approved successfully.")
        navigate("/Admin_home/PolicyList")
      }
    } catch (error) {
...
export default PolicyApprovalRejection_page
